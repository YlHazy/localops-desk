...
Public-source security hardening release.

- Requires a per-installation random bearer token for every local API route; Electron, the Edge compatibility launcher, Vite development proxy, smoke tools, and the Codex MCP bridge supply it without putting it in query parameters or build output.
- Rejects request bodies above 64 KiB by received bytes and stops retaining chunks after the limit is crossed.
- Applies one process-wide three-operation gate to outbound HTTP probes, allowlisted SSH collection, deep diagnostics, and the gated Nginx action path.
- Removes historical private-environment denylist values from tracked source and excludes test files from the Electron ASAR.
- Adds package checks for test files, runtime data, generated tokens, and release-only private markers.

The Windows x64 portable executable remains unsigned. Verify its SHA-256 value from the GitHub Release before running it.
