...
The full desk also performs a 30-second read-only sync of status, recent checks, the current report, and scheduler runtime state. Its watch rail distinguishes syncing, current, and paused states. A failed background sync keeps the last trustworthy result visible and offers an explicit retry; it does not overwrite scheduler values currently being edited or trigger a server check.
