...
While open, pet mode reads the aggregate local status every 30 seconds without triggering a server check. One shared evidence clock drives the rendered pet, expiry notifications, and Codex discussion summary, so a long-lived window cannot keep sharing an initially healthy claim after evidence becomes stale. System anomaly notifications are explicit opt-in and fire only after a later status deterioration, evidence expiry, or an increased affected count. Notification text contains aggregate counts only: no host name, address, command, or raw evidence. A one-hour quiet mode suppresses system popups without stopping status synchronization and leaves an aggregate receipt inside the pet when deterioration occurs. Clicking a delivered notification opens the full desk focused on the current highest-priority host; the local focus identifier stays in the URL fragment and is never sent to the API. The preference stays in the current browser profile (the launcher uses an isolated profile) and can be disabled from the pet at any time.
...
After `npm run build`, Windows users can run `npm run pet:window` to open the companion as an isolated Microsoft Edge app window. The launcher reuses an API only after its loopback manifest and bounded status contract identify it as LocalOps; otherwise it fails closed. Before spawning Edge it reads only aggregate pet presence and refuses an ordinary duplicate window without exposing session IDs. When no API is running, it starts its own process and stops only that owned process after the random pet session stops sending its local presence heartbeat. The pet labels this as “随窗值守” and explains that closing the window ends the owned local watch after the bounded presence grace period. If LocalOps was already running independently, the pet labels “独立服务” and explains that closing only the window leaves that service running. It does not invent a tray or hidden background mode. This remains correct when Edge hands the app request from a short-lived bootstrap process to another browser process. A page that never loads fails closed within 10 seconds; a crashed page expires from memory without persisting a session token. Stopping the launcher also closes its own browser child when that child still exists. The isolated Edge profile disables extensions and stays under ignored local data. Use `npm run pet:window:check` to verify the browser, build, URL, and API state without opening a window. Launcher-opened pets request desktop topmost by default and expose a visible “取消置顶” switch. The native helper requires no administrator rights, accepts only an active random pet session, matches exactly one Edge window titled `LocalOps Guardian`, and changes only that window's Z-order. A missing or ambiguous window fails closed and leaves an ordinary Edge app window.
