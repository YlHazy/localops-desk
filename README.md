...
Open `http://127.0.0.1:5177` after both development processes start. Vite adds the per-installation API credential only at the local proxy boundary; it is never compiled into the browser bundle. The packaged desktop app and compatibility launcher create and supply the same kind of local credential automatically.
...
The plugin expects LocalOps at `http://127.0.0.1:4317` and automatically reads the same current-user token file created by the desktop app or compatibility launcher. `LOCALOPS_API_TOKEN` and `LOCALOPS_API_TOKEN_FILE` are explicit overrides; the token is sent only as a bearer header. See `docs/03-architecture/codex-pet-integration.md` for the trust model and supported boundary.
...
the genuine-alpha matte 2D cutout at
`src/assets/localops-sentry-otter-2d.png`; it is a UI character asset, not an
installable Codex sprite. The original concept remains a direction reference,
and official upload stays blocked until a genuine transparent 1536 × 1872
asset is produced through the supported Codex pet flow.
