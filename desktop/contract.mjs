...
import { bearerHeaders } from "../server/local-auth.mjs";
...
export function desktopPetUrl(sessionId = randomUUID(), port = 4317, token = "") {
  return authenticatedUrl(`${desktopLoopbackOrigin(port)}/?mode=pet&session=${encodeURIComponent(sessionId)}&runtime=desktop`, token);
}

export function desktopCodexPetUrl(port = 4317, token = "") {
  return authenticatedUrl(`${desktopLoopbackOrigin(port)}/?mode=codex-pet&runtime=desktop`, token);
}

export function desktopCodexPanelUrl(port = 4317, token = "") {
  return authenticatedUrl(`${desktopLoopbackOrigin(port)}/?mode=codex-panel&runtime=desktop`, token);
}
...
export function desktopDeskUrl(path = "/", port = 4317, token = "") {
  ...
  target.search = "";
  return authenticatedUrl(target.toString(), token);
}
...
export async function localOpsReady(fetchImpl = fetch, port = 4317, token = "") {
  try {
    const origin = desktopLoopbackOrigin(port);
    const request = { signal: AbortSignal.timeout(1_500), headers: bearerHeaders(token) };
