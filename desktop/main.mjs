...
const runtimeDataDir = join(app.getPath("userData"), "data");
const apiToken = ensureLocalApiToken(runtimeDataDir);
...
function createPetWindow() {
  ...
  petLoadPromise = petWindow.loadURL(desktopPetUrl(undefined, runtimePort, apiToken));
  if (!smokeCheck) {
...
function createCodexPanelWindow() {
  ...
  void codexPanelWindow.loadURL(desktopCodexPanelUrl(runtimePort, apiToken)).catch((error) => {
...
function createCodexPetWindow() {
  ...
  void codexPetWindow.loadURL(desktopCodexPetUrl(runtimePort, apiToken)).then(() => codexPetWindow?.show()).catch((error) => {
...
function showDesk(path = "") {
  const url = desktopDeskUrl(path || "/", runtimePort, apiToken);
...
async function waitForApi(attempts = 60) {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    if (await localOpsReady(fetch, runtimePort, apiToken)) return true;
...
async function ensureApi() {
  if (await localOpsReady(fetch, runtimePort, apiToken)) return "existing";
  ownedApi = utilityProcess.fork(appPath("server", "index.mjs"), [], {
    env: {
      ...process.env,
      LOCALOPS_API_HOST: "127.0.0.1",
      LOCALOPS_API_PORT: String(runtimePort),
      LOCALOPS_DATA_DIR: runtimeDataDir,
      LOCALOPS_API_TOKEN: apiToken
    },
    stdio: "pipe",
