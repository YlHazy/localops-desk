...
- Every `/api/**` request requires a per-installation random bearer token. Tokens are stored outside tracked source, never accepted from a query string or body, and never emitted in API responses or logs.
...
- Outbound HTTP probes, allowlisted SSH collection, deep diagnostics, and gated recovery steps share one process-wide concurrency limit of 3.
- Timeouts:
