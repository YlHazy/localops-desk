...
function startMcp(url, extraEnv = {}) {
  const child = spawn(process.execPath, [serverScript], { env: { ...process.env, LOCALOPS_URL: url, LOCALOPS_API_TOKEN: testApiToken, ...extraEnv }, stdio: ["pipe", "pipe", "pipe"] });
...
test("exposes bounded LocalOps tools and calls only expected API routes", async (t) => {
  ...
  assert.ok(mock.requests.every((request) => request.authorization === `Bearer ${testApiToken}`));
});
