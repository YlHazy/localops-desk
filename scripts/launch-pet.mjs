...
import { bearerHeaders, ensureLocalApiToken } from "../server/local-auth.mjs";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const defaultHost = "127.0.0.1";
const defaultPort = 4317;

export function petUrl({ host = defaultHost, port = defaultPort, sessionId = null, runtimeMode = null, token = "" } = {}) {
  ...
  const target = new URL(petModePath(sessionId, runtimeMode), `http://${authority}`);
  if (token) target.hash = new URLSearchParams({ access_token: token }).toString();
  return target.toString();
}

function authRequest(url, init = {}) {
  const token = new URL(url).hash ? new URLSearchParams(new URL(url).hash.slice(1)).get("access_token") : "";
  return { ...init, headers: { ...(token ? bearerHeaders(token) : {}), ...(init.headers || {}) } };
}
...
export async function localOpsReady(url, fetchImpl = fetch) {
  try {
    const request = authRequest(url, { signal: AbortSignal.timeout(1_500) });
...
export async function petSessionPresent(url, fetchImpl = fetch) {
  try {
    ...
    const response = await fetchImpl(new URL(petPresencePath(sessionId), target), authRequest(url, { signal: AbortSignal.timeout(1_500) }));
...
export async function activePetPresence(url, fetchImpl = fetch) {
  let response;
  try {
    response = await fetchImpl(new URL("/api/pet-presence", url), authRequest(url, { signal: AbortSignal.timeout(1_500) }));
  } catch {
...
export async function launchPet({ checkOnly = false, sessionId = randomUUID() } = {}) {
  ...
  const token = ensureLocalApiToken(join(root, "data"));

  const baseUrl = petUrl({ token });
  ...
  const url = petUrl({ sessionId, runtimeMode: petRuntimeModeForApi(alreadyRunning), token });

  let apiProcess = null;
  if (!alreadyRunning) {
    apiProcess = spawn(process.execPath, [join(root, "server", "index.mjs")], {
      cwd: root,
      env: { ...process.env, LOCALOPS_API_HOST: defaultHost, LOCALOPS_API_PORT: String(defaultPort), LOCALOPS_API_TOKEN: token },
      stdio: "inherit",
      windowsHide: true
    });
    try {
