...
async function verifyRuntime() {
  ...
  try {
    ...
    const [home, manifest, status] = await Promise.all([
      fetch(`${base}/`),
      authenticatedFetch(`${base}/api/agent/manifest`).then((item) => item.json()),
      authenticatedFetch(`${base}/api/status`).then((item) => item.json())
    ]);
    ...
    const practiceStatus = await authenticatedFetch(`${base}/api/status`).then((item) => item.json());
    ...
    const [clearedStatus, clearedChecks] = await Promise.all([
      authenticatedFetch(`${base}/api/status`).then((item) => item.json()),
      authenticatedFetch(`${base}/api/checks`).then((item) => item.json())
    ]);
