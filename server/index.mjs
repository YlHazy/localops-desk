...
import { ensureLocalApiToken, requestHasValidToken } from "./local-auth.mjs";
...
const apiToken = ensureLocalApiToken(dataDir);
...
function readBody(req) {
  return new Promise((resolve, reject) => {
    ...
    if (Number.isFinite(declaredLength) && declaredLength > 1024 * 64) failTooLarge();
    req.on("data", (chunk) => {
      if (settled) return;
      const value = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
      bytes += value.length;
      if (bytes > 1024 * 64) return failTooLarge();
      chunks.push(value);
    });
    req.on("end", () => {
      if (settled) return;
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (!requestHasValidToken(req, apiToken)) {
      res.setHeader("www-authenticate", "Bearer");
      return json(res, { error: "AUTH_REQUIRED", message: "LocalOps API authentication is required." }, 401);
    }
