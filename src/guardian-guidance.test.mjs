...
test("resource warning explains why a reachable service still needs attention", () => {
  ...
  assert.match(guidance.reason, /资源使用接近设定上限/);
  assert.match(guidance.detail, /重新检查/);
  assert.match(guidance.avoid, /不要重启/);
});
...
test("entry warning takes priority when resource pressure is also present", () => {
  ...
  assert.match(guidance.reason, /网页\/API 返回异常状态/);
...
test("guidance prioritizes failed entry evidence and blocks speculative mutation", () => {
  ...
  assert.match(guidance.reason, /网页\/API 无法访问/);
  assert.match(guidance.title, /排查原因/);
  assert.match(guidance.avoid, /不要重启、部署或修改配置/);
});
...
test("expired and partial evidence never becomes a broad healthy claim", () => {
  ...
  assert.match(expired.reason, /上次检查已过期/);
  assert.match(expired.avoid, /不要重启/);
  ...
  assert.match(partial.reason, /未配置的项目仍显示未知/);
  assert.doesNotMatch(partial.reason, /均正常/);
});
